...
apdata <- read_csv(paste0(path_in, "database/ap_pred/apdata.csv"), guess_max = 10000) %>%
  dplyr::select(-st_type, -area_type, 
                ...
                -O3, -PM2.5, -PM10, -PM2.5_pemisdist, -PM10_pemisdist, # Not relevant
                -CAMSrean_o3, -CAMSrean_pm10, -CAMSrean_pm25, # Not relevant
                -CAMSanaly_o3, -CAMSanaly_pm25, -CAMSanaly_pm10, # Not relevant 
                -maiac_terra, -maiac_aqua, -omi_o3, -tropomi_o3, -dust, # Not relevant 
...
apdata <- mutate(apdata, 
  ...
  dplyr::select(-CAMSrean_no2, -CAMSanaly_no2) %>%
...
write_rds(mod, paste0(path_out, "no2_model.rds"))
write.csv(varimp, paste0(path_out, "no2_varimp.csv"))

# Variogram ----

# Prepare
apdata$res <- apdata$NO2 - predict(mod)
sfres <- st_as_sf(apdata, coords = c("x", "y"), crs = 25831) %>%
  dplyr::select(NO2, res, ID, date, yday, julian) 
# Empirical
empvar <- variogram(res~as.factor(julian), sfres, cutoff = 25000, dX = 0)
apdata$res <- NULL

# Fit pooled
fitvar <- vgm(psill=3, "Exp", range=15000, nugget=2)
fitvar <- fit.variogram(empvar, fitvar, fit.method = 6)
png(paste0(paste0(path_out, "no2_pooledvar.png")), res = 300, width = 2000, height = 1200)
plot(empvar, fitvar)
dev.off()
write_rds(fitvar, paste0(paste0(path_out, "no2_variog.rds")))
write_rds(sfres, paste0(paste0(path_out, "no2_resid.rds")))
...
empvar_st <- variogramST(res~1, STSdata, tlags=0:5, cutoff = 25000)
png(paste0(paste0(path_out, "no2_STvar.png")), res = 300, width = 2000, height = 1200)
...
for(k in 1:10){
  ...
  data_train$res1 <- data_train$NO2 - data_train$pred1
  ...
  # Process
  data_test$k <- k
  data_test <- dplyr::select(data_test, ID, date, yday, julian, k, NO2, pred1, pred2)
...
write_csv(results, paste0(path_out, "no2_perf.csv"))
